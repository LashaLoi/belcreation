...
const items = [
  {
    ...
    Icon: ({ isActive }: { isActive: boolean }) => (
      <svg
        ...
        height="1em"
        viewBox="0 0 576 512"
        className={`${isActive ? "fill-purple-800" : ""}`}
      >
          ...
          {items.map(({ path, Icon, text }) => {
            ...
            return (
              <Link
                key={path}
                href={path}
                className={`font-medium tracking-wide transition-colors duration-200 hover:text-deep-purple-accent-400 flex gap-2 items-center ${
